...
nof_mic_pos = length(nonzeros(any(Obj.Midx,2)));
...
if nargin < 3
    ...
    if nargin < 2
        ...
        lpk_pos_idx = input(sprintf('Enter the desired loudspeaker position (1->%i): [all positions associated to microphone position %i] \n',length(nonzeros(Obj.Midx(mic_pos_idx,:))),mic_pos_idx));
    
        if isempty(lpk_pos_idx)
             lpk_pos_idx = nonzeros(Obj.Midx(mic_pos_idx,:)); 
        elseif lpk_pos_idx < 1 || lpk_pos_idx > length(nonzeros(Obj.Midx(mic_pos_idx,:)))            
            error('The desired loudspeaker position must be an integer between 1 and %i!',length(nonzeros(Obj.Midx(mic_pos_idx,:))));
        end
...
IR = Obj.Data.IR(Obj.Midx(mic_pos_idx,lpk_pos_idx),:,:,:);